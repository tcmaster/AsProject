...
/**
 * @Descripton 自定义RadioGroup
 * @author LiXiaoSong
 * @2015-7-1
 * @Tonight8
 */
public class CustomerRadioGroup extends LinearLayout {
	 ...
	 * {@inheritDoc}
	 */
	public CustomerRadioGroup(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	/** 增加group的item */
	public void addGoodsOrderItem(List<GoodsSpecification> items) {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		if (child instanceof RadioButton) {
			...
			List<RadioButton> buttons = findRadioButtons((ViewGroup) child);
	...
	public List<RadioButton> findRadioButtons(ViewGroup group) {
		List<RadioButton> rbs = new ArrayList<RadioButton>();
		int len = group.getChildCount();
		for (int i = 0; i < len; i++) {
			if (group.getChildAt(i) instanceof RadioButton) {
				rbs.add((RadioButton) group.getChildAt(i));
			} else if (group.getChildAt(i) instanceof ViewGroup) {
				rbs.addAll(findRadioButtons((ViewGroup) group.getChildAt(i)));
			}
		}
		return rbs;
	}
	...
	private class PassThroughHierarchyChangeListener implements
		...
		public void onChildViewAdded(View parent, View child) {
			if (parent == CustomerRadioGroup.this
