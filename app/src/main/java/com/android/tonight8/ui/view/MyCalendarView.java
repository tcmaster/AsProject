...
public class MyCalendarView extends View implements View.OnTouchListener {
	...
	private boolean isHaveDay(String[] dayStrings, int day) {
		try {
			calendar.setTime(curDate);
			...
			if (dayStrings != null) {
				for (int i = 0; i < dayStrings.length; i++) {
					...
					if (Integer.valueOf(dayArr[0]) == year && Integer.valueOf(dayArr[1]) == month
							&& Integer.valueOf(dayArr[2]) == day) {
						return true;
					}
				}
			}
	...
	private void drawCellText(Canvas canvas, int index, String text, int color) {
		...
		float cellY = surface.monthHeight + surface.weekHeight + (y - 1) * surface.cellHeight + surface.cellHeight * 3
				/ 4f;
	...
	private void drawCellBg(Canvas canvas, int index, int color) {
		...
		canvas.drawRect(left, top, left + surface.cellWidth - surface.borderWidth, top + surface.cellHeight
	...
	private class Surface {
		...
		public void init() {
			...
			cellWidth = width / 7f;
			borderPaint = new Paint();// 格子线画笔
			borderPaint.setColor(borderColor);
			borderPaint.setStyle(Paint.Style.STROKE);
			borderWidth = (float) (0.9 * density);// 格子线画笔宽度
			...
			monthChangeBtnPaint.setStyle(Paint.Style.STROKE);
